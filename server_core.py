...
HF4_CONTENT_BUILDS = {HF4_CONTENT_BUILD, "0.8.7-HF4-GRADE-TARGET-DEDUP-R4", "0.8.7-HF4-GRADE-TARGET-DEDUP-R4-DEDUP-R4", "0.8.7-HF4-GRADE-TARGET-DEDUP-R5", "0.8.7-HF4-GRADE-TARGET-DEDUP-SINGLE-PASS-R6", "0.8.7-HF4-GRADE-TARGET-DEDUP-SINGLE-PASS-R6.1", "0.8.7-HF4-GRADE-TARGET-DEDUP-SINGLE-PASS-R6.2", "0.8.7-HF5-COMMENT-ACK"}
...
async def classroom_bridge_http_complete(request: Request):
                ...
                str(
                    ...
                    or (
                        (
                            ...
                            else "bridge_incompatible_read_result: actualiza y recarga SieRoom Bridge v0.8.7."
                        )
                        if incompatible
    ...
    if result.get("browser_followup_done") is True:
        ...
        if result.get("method") != "dom-v0.8.7":
...
def classroom_capabilities() -> str:
    ...
    return _ok({
        ...
        "official_api_limits": {
            "private_submission_comments": "La API oficial no expone lectura/escritura de comentarios privados. v0.8.7 usa un puente local con lectura verificada, reutilización segura de la misma entrega, selector de publicación estricto y guardias de cuenta/destino, sin enviar cookies de Google a Render.",
...
def classroom_private_feedback(
    ...
    if action == "status":
        return _ok({
            ...
            "note": "El puente v0.8.7 debe estar abierto en Chrome y configurado con el correo docente correcto de Classroom.",
        })
    if action == "job":
        job = bridge_queue.get(job_id)
...
def sieweb_academics(action: str, payload_json: str = "{}", confirmed: bool = False) -> str:
    """Registro académico de SieWeb y BITÁCORA DOCENTE. action: login_status|resolve_class_context|gradebook|gradebook_by_section|gradebook_summary|find_students|find_criteria|get_criteria|criteria_write_preflight|upsert_criteria_verified|build_grade_records|save_grades_verified|update_grades|get_conclusion|get_conclusions_batch|save_conclusion|save_conclusions_batch|bitacora_policy|bitacora_status|bitacora_resolve_student|bitacora_student_history|bitacora_student_report|bitacora_append_observation|bitacora_append_observations_batch|bitacora_append_academic. Las consultas de historial/informe admiten fecha_desde y fecha_hasta. Las escrituras de bitácora son append-only y exigen confirmed=true; el lote se valida completo y omite referencias existentes. Una orden explícita “BITÁCORA: …” constituye autorización. v0.8.4 crea desempeños con el contrato nativo del modal y bloquea Nivel de Logro. Para notas nuevas prefiere save_grades_verified. v0.8.12 permite cambiar abreviaturas conservando IDs, padres, descripciones y notas."""
    action = action.strip().lower(); p = _json_obj(payload_json, {})
    if action.startswith("bitacora_"):
        from bitacora import dispatch
        return json.dumps(dispatch(action, p, confirmed), ensure_ascii=False, default=str)
...
def workflow_private_comment_grades_to_sieweb(p: dict[str, Any]) -> str:
    ...
    for submission in submissions:
        ...
        if not result_valid:
            blockers.append({"submission_id": submission_id, "job_id": job.id, "reason": "read_result_not_v087_scoped"})
            continue
        scoped_read_count += 1
        if not code:

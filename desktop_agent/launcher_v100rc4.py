...
def _one_cell_verified_write_rc4(probe) -> None:
    page = probe._page
    cell_map = probe._cell_map

    print("\n============================================================")
    print(" SIEROOM 1.0 RC4 — UNA SOLA ESCRITURA REAL VERIFICADA")
    print("============================================================")
    print("RC4 puede modificar exactamente UNA celda, pero solo después de:")
    print("1) validar DOM + matrícula + API; 2) mostrar el destino real;")
    print("3) exigir una frase de confirmación exacta; 4) releer SIEweb.")
    print("RC4 NO sobrescribe una nota distinta ya existente.")
    ...
    plan_id = f"SIEWEB-RC4-{datetime.now().strftime('%Y%m%d-%H%M%S')}-{uuid.uuid4().hex[:8]}"
    evidence_dir = legacy.app_data_root() / "evidence"
    preflight_payload = {
        "plan_id": plan_id,
        "created_at": datetime.now().isoformat(timespec="seconds"),
        "mode": "ONE_CELL_VERIFIED_WRITE_RC4",
        "write_sent": False,
    ...
    print("Alcance RC4: exactamente UNA celda; sin notificación masiva.")
    ...
    print("\nPRUEBA RC4 TERMINADA. La ventana queda abierta para revisar el resultado.")
    legacy.prompt("Presiona ENTER cuando quieras cerrar el agente...")
    raise SystemExit(0)


def print_grade_cell_probe_rc4(probe) -> None:
    print("\n--- PRECHECK DE LIBRETA ---")

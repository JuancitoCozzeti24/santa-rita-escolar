...
# ---------------------------------------------------------------------------
_R3_BARBARA_COURSE_ID = "794101973737"
_R3_BARBARA_WORK_ID = "874845898173"
_R3_BARBARA_NAME = "Barbara Rafaela ALVAREZ QUEVEDO"

def _r3_wait_read(job_id: str, timeout_seconds: int = 90):
...
def _r3_wait_delete(job_id: str, timeout_seconds: int = 90):
    deadline = _time.time() + timeout_seconds
    while _time.time() < deadline:
        job = _private_comment_delete_queue.get(job_id)
...
def _r3_structured(result):
    ...
    for item in comments:
        ...
        if (
            ...
            out.append({
                "text": text,
                "domOrder": item.get("domOrder"),
                "characterCount": len(text),
            })
    return out

def _r3_barbara_worker():
    if not str(os.getenv("SIEROOM_R3_BARBARA_TOKEN") or "").strip():
        return
    print("R3 BARBARA: inicio protegido.", flush=True)
    try:
        students = classroom.list_students(_R3_BARBARA_COURSE_ID)
        matches = [s for s in students if str(s.get("name") or "").strip() == _R3_BARBARA_NAME]
        if len(matches) != 1:
            print(f"R3 BARBARA ERROR: coincidencias_estudiante={len(matches)}.", flush=True)
            return
        user_id = str(matches[0].get("userId") or "")
        submissions = classroom.list_submissions(_R3_BARBARA_COURSE_ID, _R3_BARBARA_WORK_ID)
        subs = [s for s in submissions if str(s.get("userId") or "") == user_id]
        if len(subs) != 1:
            print(f"R3 BARBARA ERROR: coincidencias_entrega={len(subs)}.", flush=True)
            return
        sub = subs[0]
        sid = str(sub.get("id") or "")
        url = str(sub.get("alternateLink") or "")
        if not sid or not url:
            print("R3 BARBARA ERROR: entrega sin id/url.", flush=True)
            return

        read = bridge_queue.enqueue(
            course_id=_R3_BARBARA_COURSE_ID,
            course_work_id=_R3_BARBARA_WORK_ID,
            submission_id=sid,
            submission_url=url,
            operation="read_private_comments",
        )
        current = _r3_wait_read(read.id)
        if not current or current.status != "completed":
            print(f"R3 BARBARA ERROR: lectura status={getattr(current,'status',None)} error={getattr(current,'error',None)}.", flush=True)
            return

        comments = _r3_structured(current.bridge_result or {})
        print(f"R3 BARBARA: comentarios_estructurados={len(comments)}.", flush=True)
        if len(comments) != 2:
            print("R3 BARBARA RESULTADO: no hay exactamente 2; no se borra nada.", flush=True)
            return

        a,b = comments
        if int(a["characterCount"]) == int(b["characterCount"]):
            print("R3 BARBARA RESULTADO: empate de longitud; no se borra nada.", flush=True)
            return
        keeper,target = (a,b) if int(a["characterCount"]) > int(b["characterCount"]) else (b,a)
        print(f"R3 BARBARA: conservar={keeper['characterCount']} borrar={target['characterCount']}.", flush=True)
        ...
        deleted = _r3_wait_delete(dj.id)
        print(f"R3 BARBARA: delete_status={getattr(deleted,'status',None)} delete_error={getattr(deleted,'error',None)}.", flush=True)
        ...
        v = _r3_wait_read(verify.id)
        if not v or v.status != "completed":
            print(f"R3 BARBARA ERROR: verificacion status={getattr(v,'status',None)} error={getattr(v,'error',None)}.", flush=True)
            return
        after = _r3_structured(v.bridge_result or {})
        keeper_norm = _delete_norm_text(str(keeper["text"]))
        clean = len(after) == 1 and _delete_norm_text(str(after[0]["text"])) == keeper_norm
        print(
            f"R3 BARBARA VERIFICACION: despues={len(after)} keeper_confirmado={clean}.",
            flush=True,
        )
        if clean:
            print(f"R3 BARBARA OK: se conservó {keeper['characterCount']} y se eliminó {target['characterCount']}.", flush=True)
        else:
            print("R3 BARBARA ERROR: resultado no limpio; no ejecutar nada más.", flush=True)
    except Exception as exc:
        print(f"R3 BARBARA ERROR FATAL: {type(exc).__name__}: {exc}", flush=True)

if str(os.getenv("SIEROOM_R3_BARBARA_TOKEN") or "").strip():
    Thread(target=_r3_barbara_worker, name="sieroom-r3-barbara", daemon=True).start()

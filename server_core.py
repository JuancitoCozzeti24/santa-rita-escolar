...
def sieweb_academics(action: str, payload_json: str = "{}", confirmed: bool = False) -> str:
    """Registro académico de SieWeb y BITÁCORA DOCENTE. action: login_status|resolve_class_context|gradebook|gradebook_by_section|gradebook_summary|find_students|find_criteria|get_criteria|criteria_write_preflight|upsert_criteria_verified|build_grade_records|save_grades_verified|update_grades|get_conclusion|get_conclusions_batch|save_conclusion|save_conclusions_batch|bitacora_policy|bitacora_status|bitacora_resolve_student|bitacora_student_history|bitacora_student_report|bitacora_append_observation|bitacora_append_academic. Las consultas de historial/informe admiten fecha_desde y fecha_hasta. Las escrituras de bitácora son append-only y exigen confirmed=true; una orden explícita “BITÁCORA: …” constituye autorización. v0.8.4 crea desempeños con el contrato nativo del modal y bloquea Nivel de Logro. Para notas nuevas prefiere save_grades_verified. v0.8.12 permite cambiar abreviaturas conservando IDs, padres, descripciones y notas."""
    action = action.strip().lower(); p = _json_obj(payload_json, {})

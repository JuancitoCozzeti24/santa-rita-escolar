...
"""
if "from bitacora import (" not in s:
    if old_import not in s:
        raise SystemExit("config import anchor not found")
    s = s.replace(old_import, new_import, 1)

rule_anchor = "- Distingue información general de información privada. No reveles notas, conducta, observaciones, correos, datos personales ni historial de otro estudiante.\n"
rule_extra = """- La BITÁCORA DOCENTE es una fuente de memoria conductual y de seguimiento, pero solo se usa para el propio estudiante cuando este se identifica como tal en la consulta. Nunca la uses para responder sobre un compañero.
...
"""
if rule_extra.strip() not in s:
    if rule_anchor not in s:
        raise SystemExit("base instructions anchor not found")
    s = s.replace(rule_anchor, rule_anchor + rule_extra, 1)
...
def _bitacora_memory_context(
    ...
    return "\n".join(lines), meta
'''
if "_bitacora_memory_context(" not in s:
    if helper_anchor not in s:
        raise SystemExit("helper anchor not found")
    s = s.replace(helper_anchor, helper + helper_anchor, 1)

body_anchor = '        message = str(body.get("message") or "").strip()\n        grade = str(body.get("grade") or "").strip()\n'
body_new = body_anchor + '        student_hint = str(body.get("student") or "").strip()\n        section = str(body.get("section") or "").strip()\n'
if 'student_hint = str(body.get("student")' not in s:
    if body_anchor not in s:
        raise SystemExit("body anchor not found")
    s = s.replace(body_anchor, body_new, 1)

context_anchor = '''        core, core_sources = _core_context(message)
        classroom, classroom_sources = _classroom_context(grade, message)
        drive, drive_sources = _drive_search_context(message)

        source_blocks = []
'''
context_new = '''        core, core_sources = _core_context(message)
        classroom, classroom_sources = _classroom_context(grade, message)
        drive, drive_sources = _drive_search_context(message)
...
'''
privacy_new = '''        if _looks_sensitive(message) and not bitacora_meta.get("resolved"):
            context += (
                "\\n\\n## RESTRICCIÓN DE PRIVACIDAD\\n"
...
'''
if 'not bitacora_meta.get("resolved")' not in s:
    if privacy_old not in s:
        raise SystemExit("privacy anchor not found")
    s = s.replace(privacy_old, privacy_new, 1)

fallback_anchor = '''        if not reply:
            reply = _fallback(message, grade, core)

        return JSONResponse({
'''
fallback_new = '''        if not reply:
        ...
        return JSONResponse({
'''
if "No tienes incidencias ni observaciones conductuales registradas" not in s:
    if fallback_anchor not in s:
        raise SystemExit("fallback anchor not found")
    s = s.replace(fallback_anchor, fallback_new, 1)

meta_anchor = '''                "drive_sources": drive_sources,
                "private_student_data_used": False,
                "updated_at": _now_lima().isoformat(),
'''
meta_new = '''                "drive_sources": drive_sources,
                "bitacora_memory": bitacora_meta,
                "private_student_data_used": bool(bitacora_meta.get("resolved") and bitacora_meta.get("records", 0) > 0),
                "updated_at": _now_lima().isoformat(),
'''
if '"bitacora_memory": bitacora_meta' not in s:
    if meta_anchor not in s:
        raise SystemExit("meta anchor not found")
    s = s.replace(meta_anchor, meta_new, 1)

status_anchor = '''        "classroom_oauth_configured": bool(settings.google_client_id and settings.google_client_secret and settings.google_refresh_token),
        "privacy_mode": "family_student_read_only",
'''
status_new = '''        "classroom_oauth_configured": bool(settings.google_client_id and settings.google_client_secret and settings.google_refresh_token),
        "bitacora_memory_configured": True,
        "privacy_mode": "family_student_read_only",
'''
if '"bitacora_memory_configured": True' not in s:
